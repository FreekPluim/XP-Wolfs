...
async function HandleCheck(interaction, guild, client, options){
    ...
    if(!foxData.specialFunctions[0].canCheck){
        gen.reply(interaction, "You can't check people at this time.");
        return;
    }
    if(!foxData.specialFunctions[0].hasAbility){
    ...
    await gen.SendFeedback(guild.id, "The Fox", "The fox is going to check " + userMention(options.getUser("target-one").id) + ", " + userMention(options.getUser("target-two").id) + " and " + userMention(options.getUser("target-three").id) + " tonight", client)
...
async function ResolveCheck([client, game])
{
    ...
    if(vilagers.factionMembers.includes(toCheck[0]) && vilagers.factionMembers.includes(toCheck[1]) && vilagers.factionMembers.includes(toCheck[2]))
    {
        ...
        await gen.SendToChannel(fox.channelID, "All vilagers", "All of the people you checked are vilagers... You feel your power fade away", client);
        ...
        await gen.SendToChannel(fox.channelID, "Imposter", "One or more of the people you checked is not a vilager", client);
        return;
    }
