...
async function listenIn([client, game])
{
    ...
    let messages = [];
    WWChannel.awaitMessages({filter: m_filter, max: 1, time: (60000 * 60), errors: ["time"]})
        .then(async (collection) => {
            messages.push(collection.first());
            await WWChannel.awaitMessages({filter: m_filter, time: (60000 * 15)})
                .then(async (collection2) => {
                    ...
                    await messages.forEach(async message => {
                        ...
                        if(CultistRole != undefined && CultistRole.roleMembers.includes(message.author.id)){
                ...
                .catch(async (err) => {
                    ...
                    if(CultistRole != undefined && CultistRole.roleMembers.includes(messages[0].author.id)){
                        sendMessage += "**wolf " + (WWRole.roleMembers.length + 1) + "**: " + messages[0].content + "\n";
                    }

                    await gen.SendToChannel(role.channelID, "Results", sendMessage, client);
                    await gen.SendToChannel(WWRole.channelID, "Russing outside", "You feel like there was someone listening but they are gone now", client)
                });
    }).catch(async (err) => {
        //console.log(err)
