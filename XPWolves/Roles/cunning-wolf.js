...
async function handleCunning(interaction, guild, options){
    const {client} = interaction
    const cunning = await getters.GetRole(guild.id, "cunning-wolf")
    const user = await users.findOne({guildID: guild.id, _id: options.getUser("target").id});
    ...
    await rolesSchema.updateOne({guildID: guild.id, roleName: "cunning-wolf"}, {$set: {"specialFunctions.0.cunningWolfVote": options.getUser("target").id}})

    await gen.SendToChannel(cunning.channelID, 
        "Fake Vote", "You will fake vote for " + userMention(options.getUser("target").id) + " today! \n" +
