...
async function handleVote(options, guild, interaction){
    ...
    //Handle cunning wolf
    if(isCunning){
        gen.SendToChannel(game.channels[0].voteChannel, "LYNCH VOTE", "**" + gen.getName(interaction, interaction.user.id) + "** has voted on **" + gen.getName(interaction, votedPerson) + "** to lynch", client)
...
async function handleVoteAbstained(guild, interaction, client){
    ...
    if(game && game.canVote)
    {
        ...
        await gen.SendToChannel(game.channels[0].voteChannel, "LYNCH VOTE", "**" + gen.getName(interaction, interaction.user.id) + "** has abstained from voting", client)
...
async function handleVoteEnd([client, game]){
    ...
    //Reveal votes
    const cunning = await getters.GetRole(game._id, "cunning-wolf")
    let revealString = "";
    
    await alivePlayers.forEach(async player =>{
        //Check for cunning wolf
        if(cunning.specialFunctions.length != 0 && cunning.roleMembers.includes(player.id) && cunning.specialFunctions[0].cunningWolfVote != ""){
            revealString += userMention(player._id) + " voted for " + userMention(cunning.specialFunctions[0].cunningWolfVote) + "\n"
        }
        else if(player.votedOn == "" || player.votedOn == undefined || player.votedOn == "abstained"){
